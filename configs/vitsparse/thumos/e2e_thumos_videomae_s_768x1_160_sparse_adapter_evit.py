"""
Ablation config: EViT token selector + SparseConv2D adapter.

Identical to e2e_thumos_videomae_s_768x1_160_sparse_adapter.py except
the backbone type is ``VisionTransformerSparseAdapterEViT``, which fuses
dropped tokens into a single representative token (EViT, Liang et al.,
ICLR 2022) instead of discarding them.  Everything else — keep_rate,
adapter_conv_types, adapter_use_attn — is unchanged so only the selector
strategy differs.
"""
...
model = dict(
    backbone=dict(
        ...
        backbone=dict(
            ...
            type="VisionTransformerSparseAdapterEViT",
            # ──────────────────────────────────────────────────────────────
            img_size=224,
            patch_size=16,
            embed_dims=384,
...
work_dir = (
    "exps/thumos/vitsparse/e2e_actionformer_videomae_s_768x1_160_sparse_adapter_evit"
)
