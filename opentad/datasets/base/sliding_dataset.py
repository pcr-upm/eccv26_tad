...
class SlidingWindowDataset:
    def __init__(
        ...
        # (start, end) fraction of sorted videos to use, e.g. (0.0, 0.85) for train, (0.85, 1.0) for val
    ...
    def get_dataset(self):
        ...
        # Collect all valid (video_name, video_info, video_anno) tuples first
        candidates = []
        for video_name, video_info in anno_database.items():
            ...
            candidates.append((video_name, video_info, video_anno))

        # Apply deterministic video-level split if requested
        if self.video_split_ratio is not None:
            candidates.sort(key=lambda x: x[0])
            n = len(candidates)
            start = int(self.video_split_ratio[0] * n)
            end = int(self.video_split_ratio[1] * n)
            candidates = candidates[start:end]

        self.data_list = []
        for video_name, video_info, video_anno in candidates:
