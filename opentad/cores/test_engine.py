...
def eval_one_epoch(
    ...
    # Initialize wandb
    if rank == 0:
        wandb.init(project=cfg.get("project_name", "opentad"), config=cfg)

    # load the ema dict for evaluation
    if model_ema != None:
        current_dict = copy.deepcopy(model.state_dict())
        unwrapped = model.module if hasattr(model, 'module') else model
        unwrapped.load_state_dict(model_ema.module.state_dict())
            ...
            if not training:
                ...
                for video_id, predictions in result_dict.items():
                    for pred in predictions:
                        data.append(
                            [video_id, str(pred["segment"]), pred["label"], pred["score"]]
                        )
