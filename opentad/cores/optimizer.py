...
def build_optimizer(cfg, model, logger):
    ...
    if hasattr(model_without_ddp, "backbone"):  # if backbone exists
        if model_without_ddp.backbone.freeze_backbone == False:  # not frozen
            assert (
            ...
            backbone_optim_groups = get_backbone_optim_groups(
                backbone_cfg, model, logger
            )

        else:  # frozen backbone
            backbone_optim_groups = []
            # Pop the backbone config if it exists to avoid passing it to optimizer
    ...
    if "paramwise" in cfg.keys() and cfg["paramwise"]:
        cfg.pop("paramwise")
        det_optim_groups = model_without_ddp.get_optim_groups(cfg)
    else:
